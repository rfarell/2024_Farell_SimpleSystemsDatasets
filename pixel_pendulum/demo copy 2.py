...
data = torch.arange(1, num_frames + 1).float().unsqueeze(1).unsqueeze(2).unsqueeze(3)
data = data.expand(-1, 1, height, width)
...
for epoch in range(num_epochs):
    ...
    for j in range(K):
        ...
        qp_pred = predict_full_trajectory(hamiltonian_model, qp0, T,T)
